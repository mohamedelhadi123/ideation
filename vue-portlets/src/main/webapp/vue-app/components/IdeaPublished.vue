                  ...
                  <v-expansion-panel-content v-for="d in donnes" :key="d.id">
                    ...
                    <v-card>
                      <v-card-text class="px-4 grey--text" style="    width: 95%">
                        <strong>Resumé :</strong>{{ d.resume }}<router-link :to="`/ideainfo/${d.id}`">... Lire la suite </router-link>
                        <p>
                          <br>
                         
                         <a href="#" @click="GetUrlSpace(d.title)" v-if="d.etat===false">Visiter le  projet </a>

                      
                          <button
                            v-if="(userNameConnected===d.user && d.etat!==false)"
                           ...
                           <button v-if="d.etat!==false" class="btn btn-small" @click="AffichDialog">Modifier</button>

                          <button
                            v-if="showbtn===true && d.etat!==false"
                            class="btn btn-small"
                            @click.prevent="AddToArchived(d.id,d.title,d.description,d.resume,d.explanation)">
                            Ajouter aux listes Archivée
                          </button>
...
export default {
   ...
   this.AllPublished();
         ...
         showMyIdeaPublished(){
                ...
                }).then(response=>{this.donnes=this.donnes.filter(d=>d.id !==id);
                if(this.donnes.length===0){
                    this.alt=true;
                    this.componet=false;
                }})

            },CreateEspace(id,title,description,resume,explanation,etat){
              
              this.chargement=true;
              this.disable=true;
              
              axios.post(`/portal/rest/idea/addSapace/${title}`,this.dataspace, {
                        headers: {
                            'Content-type': 'application/json',
                        }
                    }).then ( test => {
                             this.datajsonUpdate.id=id;
                             this.datajsonUpdate.title=title;
                             this.datajsonUpdate.description=description;
                             this.datajsonUpdate.resume=resume;
                             this.datajsonUpdate.explanation=explanation;
                             this.datajsonUpdate.etat=1;
                             this.datajsonUpdate.status="PUBLISHED";

                       axios.put('/portal/rest/idea/update', this.datajsonUpdate, {
                    headers: {
                        'Content-type': 'application/json',
                    }
                }).then(response=>{ const baseUrl = window.location.origin;
              ...
              axios.get('/portal/rest/idea/geturl/'+title)
              .then(response => { this.donnes=response.data;
              const baseUrl = window.location.origin;
              const urlSpace=`${baseUrl}/portal/g/:spaces:`+response.data.url+`/`+response.data.url
                ...
                axios.put('/portal/rest/idea/update', this.dataupdate, {
                    headers: {
                        'Content-type': 'application/json',
                    }
