                  ...
                  <v-expansion-panel-content v-for="d in donnes" :key="d.id">
                    ...
                    <v-card>
                      <v-card-text class="px-4 grey--text" style="    width: 95%">
                        ...
                        <p>
                          <br>
                                <button class="btn btn-small btn-primary"  v-if="d.spaceID" @click="GetUrlSpace(d.spaceID)">Visiter le projet</button>
                          ...
                          <button
                                  ...
                                  type="button"
                                  :disabled="disable"
                                  @click.prevent="CreateSpace(d.id,d.title,d.description,d.resume,d.explanation)"
                          >Créer une espace</button>

                          <button
                                  v-if="!d.spaceID && userNameConnected===d.user"
                                  class="btn btn-small"
                                  @click="AffichDialog"
                          >Modifier</button>

                          <button
                                  v-if="userNameConnected==='root' && !d.spaceID "
                                  class="btn btn-small"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      CreateSpace(id, title, description, resume, explanation, isProject) {
      ...
      GetUrlSpace(spaceID) {
        axios
                .get('/portal/rest/idea/geturl/' + spaceID)
                .then((response) => {
                  const urlSpace=response.data.url;
